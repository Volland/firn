...
COPY clojure /app/clojure
COPY --from=rustbuild /app/rust/target/release/libfirn_parser.so /app/clojure/resources/
...
RUN native-image -jar target/firn-0.0.5-SNAPSHOT-standalone.jar \
  ...
  -H:IncludeResources=libfirn_parser.dylib \
  -H:IncludeResources=libfirn_parser.so \
...
COPY --from=rustbuild /app/rust/target/release/libfirn_parser.so /home/user/.firn/libfirn_parser.so
USER user
