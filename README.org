...
- Clone the repo
- Run bin/compile to download dependencies
  - You can halt the process before GRAALVM kicks off it's compilation
  - You only really need the rust libs / compilation process to happen.
- I use Emacs and so I open `core.clj` and boot up a Cider REPL.
  + Then, I will usually test changes by iteratively running the commands within the comment block at the bottom of core, which simulates running the binary.
